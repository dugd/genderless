import type {
  IInferenceService, IDecitionTrace,
  ISessionFacade
...
export default class SessionFacade implements ISessionFacade {
    ...
    back() {
        ...
        this.events.push({ at: Date.now(), from: from.currentId, to: toCtx.currentId });
    ...
    reset() {
        ...
        this.events.push({ at: Date.now(), from: "RESET" as any, to: ctx.currentId });
        const node = this.inf.getNode(ctx.currentId);
        return { ctx, node, finished: this.inf.isFinished(ctx) };
    }
